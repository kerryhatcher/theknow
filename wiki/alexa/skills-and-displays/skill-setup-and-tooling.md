...
|---------|----------|------|-------|------------|
| **Alexa-hosted** | Python (and Node.js) | Unlimited Lambda requests, 25 GB S3 + 250 GB/month data transfer out, 25 GB DynamoDB + 10M reads/writes per month. Capped at 75 Alexa-hosted skills per developer account. | Minutes. Click "Create Skill," choose Python, edit in the console or locally. `ask deploy` uploads to Amazon's AWS account. | A quick prototype or a personal skill with no infrastructure ownership. Select the Python runtime shown by the console; do not copy an old runtime number from a tutorial. |
| **AWS Lambda** (your account) | Python | Free tier: 1M requests, 400,000 GB-seconds/month. $0.20 per 1M requests + memory-time charges after free tier. | Hours. Create AWS account, write code locally, deploy with SAM, CDK, Terraform, or a zip package. Register Lambda ARN in the Developer Console. | The recommended default for a durable skill: current runtime control, IAM, observability, VPC choices, and full ownership of data services. |
...
For a new Python skill, choose your own AWS Lambda unless the Alexa-hosted quotas and runtime shown in the console meet the whole project’s needs. Lambda removes HTTPS request-signature work and lets you pin and upgrade a currently supported Python runtime on your schedule.
...
**What to commit to Git:** `skill-package/` (skill.json and the interaction models), `lambda/` code, `requirements.txt`, infrastructure code, and `ask-resources.json`. **Do not commit:** `.ask/` (CLI-managed deployment state, regenerated by `ask deploy`), virtual environments, package build directories, any AWS credentials, or compiled artifacts. `ask configure` writes its credentials outside the project directory, so leave `.ask/` out and there is no normal scaffold secret to commit.

For the backend, use `ask-sdk` in `requirements.txt`; the canonical handler is
`lambda_handler.handler`. Package pure-Python dependencies with the function or use a layer.
If a dependency has native code, build it for the same Linux distribution and architecture as
the target Lambda runtime. See [Backend in Python](backend-python.md#package-and-deploy).
...
- [backend-python.md](backend-python.md): Write the fulfillment endpoint with the official Python SDK.
